...
export default {
  ...
  async created() {
    ...
    Promise.all([province, city, area])
      .then(result => {
        provinceData = result[0]
        cityData = result[1]
        areaData = result[2]
        this.init()
      })
  ...
  methods: {
    getProvince() {
      return new Promise(resolve => {
        this.$http.post(`/addons/xshop/area/province`, {}).then(response => {
    ...
    getCity() {
      return new Promise(resolve => {
        this.$http.post(`/addons/xshop/area/city`, {}).then(response => {
          const data = response.data;
          if (response.code == 1) {
            resolve(data);
          }
    ...
    getArea() {
      return new Promise(resolve => {
        this.$http.post(`/addons/xshop/area/area`, {}).then(response => {
          const data = response.data;
          if (response.code == 1) {
            resolve(data);
          }
