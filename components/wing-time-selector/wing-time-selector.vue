...
export default {
    ...
    // endDate(oldValue, newValue){
      
    // }
	},
	created() {
    // console.log('--this.beginDate--',this.beginDate)
    this.beginDate = this.beginDate || this.$service.dateTime().split(' ')[0] || '1900-01-01'
				...
				default:
					{
						...
						date = {
							...
							minute,
							second,
              stamp: this.years[value[0]].substring(0, 4) + month + day + hour + minute + second,
							key:
								this.years[value[0]].substring(0, 4) +
								'-' +
								(month.length == 1 ? '0' + month : month) +
