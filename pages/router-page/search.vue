  ...
  <empty setSrc="https://cdn.swh296.com/img/common/empty_content.png" v-if="loaded === true && (!(searchStore.store &&searchStore.store.name) && searchStore.product_list.total==0)">
    ...
    <scroll-view
      ...
      @scrolltolower="search"
    	scroll-y
      v-if="searchStore.store || searchList.length>0"
    >
      <view class="store" v-if="searchStore.store" @click="_gStore(searchStore.store.id)">
        <view class="store-l">
          <image :src="searchStore.store.log_image_text" mode=""></image>
          <view class="">
