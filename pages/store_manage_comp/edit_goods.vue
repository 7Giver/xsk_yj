  ...
  <view class="container">
    <view class="order-section">
        ...
        <view class="item-r">
          <view class="picker-section">
           ...
           <!--   <view v-if="isPickerdefText" class="choose-text">
              ...
              <view class="uni-input">{{ newcategoryList[index] }}</view>
            </picker>
          </view>
        </view>
      </view>
      ...
      <view class="exaddress-section">
        ...
        <view class="textarea-inner"><textarea class="textarea" maxlength="80" v-model="goodsInfo.description" placeholder="请输入商品卖点" /></view>
      </view>
      <view class="exaddress-section mb">
        ...
        <view class="goods_cs">
          <view class="goods-item">
            ...
            <view class="item-r"><input type="text" v-model="goodsInfo.parameter.brand" maxlength="11" placeholder="请输入品牌" value="" /></view>
          </view>
          <view class="goods-item">
            <view class="item-l">型号</view>
            <view class="item-r"><input type="text" v-model="goodsInfo.parameter.model" maxlength="11" placeholder="请输入型号" value="" /></view>
          </view>
          <view class="goods-item">
            <view class="item-l">颜色</view>
            <view class="item-r"><input type="text" v-model="goodsInfo.parameter.color" maxlength="11" placeholder="请输入颜色" value="" /></view>
          </view>
          <view class="goods-item">
            <view class="item-l">材质</view>
            <view class="item-r"><input type="text" v-model="goodsInfo.parameter.material" maxlength="11" placeholder="请输入材质" value="" /></view>
          </view>
        </view>
      </view>
      <!--  -->
      <view class="exaddress-section mb">
        ...
        <view class="sku">
          ...
          <view>默认</view>
          <!-- specifList -->
          <view v-for="(item, index) in sku" :class="{ active: index == specifId }" :key="index" @click="_confirmspecif(item, index)">
            {{ item.value }}
            <!-- <text  @click.stop="_deleteSpecif(item, index)" class="cuIcon-roundclosefill"></text> -->
            <image src="https://cdn.swh296.com/img/iconList/delete_active.png" v-if="item.is_delete" @click.stop="_deleteSpecif(item, index)" mode=""></image>
          </view>
      ...
      <view class="exaddress-section mb">
        ...
        <view class="goods_cs">
          <view class="goods-item">
            ...
            <view class="item-r"><input type="number" v-model="goodsInfo.market_price" placeholder="请输入原价" value="" /></view>
          </view>
          <view class="goods-item">
            <view class="item-l">折扣价</view>
            <view class="item-r"><input type="number" v-model="goodsInfo.price" placeholder="请输入折扣价" value="" /></view>
          </view>
        </view>
      </view>
      <!--  -->
      <view class="item">
        ...
        <view class="item-r">
          <!-- <view class="input-section" v-if="goodsInfo.sku"><input type="number" v-model="goodsInfo.sku[0].stock" maxlength="11" placeholder="请输入商品数量" value="" /></view> -->
          <view class="input-section" v-if="goodsInfo.sku"><input type="number" v-model.trim="goodsInfo.stock" maxlength="11" placeholder="请输入商品数量" value="" /></view>
        </view>
      </view>
<!--     <view class="item">
        <view class="item-l">
        ...
        <view class="item-r">
          <view class="input-section"><input type="text" v-model="goodsInfo.sn" maxlength="11" placeholder="请输入商品编号" value="" /></view>
        </view>
      </view> -->
      <!--  -->
...
export default {
  ...
  async onLoad(options) {
    ...
    const details = await this.detailsProducts()
    this.goodsInfo = details
    this.goodsInfo.parameter=JSON.parse(details.parameter)
    this.is_can_appoint = details.is_can_appoint //是否开启预约
    this.categoryId = details.category_id
    this.swiperList =  details.image_text.split(',') //商品轮播图
    this.saveList = details.image.split(',')
    this.sku = details.sku
    this.goodsInfo.stock = details.sku[0].stock
    this.saveListDetail = details.content_url.split(',')
    this.swiperDetailList =details.content_text.split(',')  //商品详情图文
    this.category()
  },
  methods: {
    // 新增商品规格
    ...
    _submitgoodsInfo() {
      ...
      let obj = Object.assign({id:goodsId++,is_delete:true},this.newgoods)
      console.log(obj)
      if(!this.isNewPecif){
        this.sku.push(obj)
      } else {
        console.warn('this.specifId',this.specifId)
        this.sku[this.specifId] = obj
      }
    ...
    _confirm() {
      ...
      if (this.goodsInfo.description.trim() === '') {
        this.$api.msg('请输入商品卖点');
        return;
      }
      let params = this.goodsInfo.parameter;
      ...
      if (this.goodsInfo.market_price.trim() === '') {
        this.$api.msg('请输入商品原价');
        return;
      }
      if (this.goodsInfo.price.trim() === '') {
        this.$api.msg('请输入商品折扣价');
        return;
      }
      if (this.goodsInfo.market_price - this.goodsInfo.price <= 0) {
        this.$api.msg('请输入正确的满减金额');
        return;
      }
      console.log(this.goodsInfo.stock)
      if (!this.goodsInfo.stock) {
        this.$api.msg('请输入商品数量');
        return;
      }
      // if (this.goodsInfo.sn.trim() === '') {
      ...
      this.editProducts(newsaveList, newsaveListDetail);
    },
    detailsProducts() {
      var _this = this;
      return new Promise(resolve=>{
        this.$http
          .post(`/api/merchants/Products/details`, {
            id:this.editId
          })
          .then(response => {
    ...
    editProducts(newsaveList, newsaveListDetail) {
      this.disabled = true
      var _this = this;
      uni.showLoading({
        title:'正在提交...'
      ...
      this.$http
        ...
        .then(response => {
          ...
          if (response.code === 1) {
            ...
            this.index = this.goodsInfo.category_id -1
          }
        });
    }
  }
